...
import { Filing, FilingStatus, Report } from './ipc-types'
import {
  Button,
  Container,
  FormControl,
    ...
    <TableCell align="right">
        ...
        >
          <MenuItem
            onClick={async e => {
              e.stopPropagation()
              await invoke('export_filing', { filingId: props.filing.id })
          ...
          <MenuItem
            onClick={async e => {
              e.stopPropagation()
              await invoke('delete_filing', { filingId: props.filing.id })
              queryClient.invalidateQueries({ queryKey: ['filings'] })
              setMenuAnchorEl(null)
            }}
          >
